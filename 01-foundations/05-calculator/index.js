...
const updateFontSize = () => {
  ...
  const fontSize = Number(
    window.getComputedStyle(display).fontSize.slice(0, -2),
  );
  const isOffScreen = getTextWidthEstimate(fontSize, textLength) > maxWidth;

  if (isOffScreen) {
    display.style.fontSize = `${fontSize * 0.9}px`;
  } else {
    const defaultSize =
      (Math.min(window.innerWidth, window.innerHeight) * 0.8) / 6;
    const defaultSizeFits =
      getTextWidthEstimate(defaultSize, textLength) < maxWidth;
    const increasedFontSize = Math.min(defaultSize, fontSize * 1.1);
    const incrasedSizeFits =
      getTextWidthEstimate(increasedFontSize, textLength) < maxWidth;
    const newFontSize = defaultSizeFits
      ? defaultSize
      : incrasedSizeFits
        ? increasedFontSize
        : fontSize;
